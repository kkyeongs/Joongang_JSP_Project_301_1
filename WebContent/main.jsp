...
<body>
    ...
    <nav class="navbar navbar-expand-lg">
        <div class="container-fluid">
            <a class="navbar-brand" href="startIndex.jsp">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav ml-auto mb-2 mb-lg-0">
                    <li class="nav-item">
                        <a class="nav-link nav-link-1 active" aria-current="page" href="index.jsp">Photos</a>
                    </li>
                    <li class="nav-item">
      ...
      <div class="tm-hero d-flex justify-content-center align-items-center" data-parallax="scroll" data-image-src="img/hero.jpg">
        <form class="d-flex tm-search-form" action="search.do">
            <input class="form-control tm-search-input" type="search" name="search" value="${search }" aria-label="Search">
      ...
      <div class="row mb-4">
            ...
            <div class="col-6 d-flex justify-content-end align-items-center">
                <form action="" class="tm-text-primary">
                   <!-- <input type="button" value="사진 업로드" id="btn"> -->
                    Page ${searchPageNum } of ${totPage }
                </form>
            </div>
        </div>
        ...
        <div class="row tm-mb-90 tm-gallery" style="margin-bottom: 30px;">
         ...
         <form class="d-flex justify-content-center" action="search.do?" style="background-color: transparent; ">
            <div class="d-flex justify-content-center h-100">
               <div class="searchbar">
                  <input class="search_input" type="search" name="search" value="${search }" aria-label="Search" autocomplete="off">
                     <input type="hidden" name="searchPageNum" value="1">
        ...
        <div class="row tm-mb-90">
         <div class="d-flex justify-content-center align-items-center tm-paging-col">
            <div class="col-md-12 text-center d-flex justify-content-center">
               <c:set var="startNum" value="${searchPageNum-(searchPageNum-1)%5 }"/>
               <c:set var="lastNum" value="${totPage }"/>
               <c:if test="${startNum > 1}">
                  <a href="search.do?search=${search}&searchPageNum=${startNum-1 }" class="tm-paging-link col-md-auto">이전</a>
               </c:if>
               <c:if test="${startNum <= 1}">
               ...
               <c:forEach var="i" begin="0" end="4">
                  <c:if test="${(startNum+i) <= lastNum }">
                     <a href="search.do?search=${search}&searchPageNum=${startNum+i }" class="tm-paging-link col-md-auto">${startNum+i }</a>
                  </c:if>
               </c:forEach>
               <c:if test="${startNum+5 < lastNum }">
                  <a href="search.do?search=${search}&searchPageNum=${startNum+5 }" class="tm-paging-link col-md-auto">다음</a>
               </c:if>
               <c:if test="${startNum+5 >= lastNum }">
