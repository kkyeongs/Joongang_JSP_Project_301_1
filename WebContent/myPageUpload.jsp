  ...
  align-items:center; float: none;">
	...
	<div class="container-fluid tm-container-content tm-mt-60">
        ...
        <div class="row tm-mb-90 tm-gallery" style="margin-bottom: 30px;">
            <c:forEach var="pic" items="${myUploadList }" varStatus="status">
			...
			<div class="d-flex justify-content-center align-items-center tm-paging-col">
				<div class="col-md-12 text-center d-flex justify-content-center">
					...
					<c:if test="${startNum > 1}">
						<a href="myPageUpload.do?myPageNum=${startNum-1 }" class="tm-paging-link col-md-auto">이전</a>
					</c:if>
					<c:if test="${startNum <= 1}">
					...
					<c:forEach var="i" begin="0" end="4">
						<c:if test="${(startNum+i) <= lastNum }">
							<a href="myPageUpload.do?myPageNum=${startNum+i }" class="tm-paging-link col-md-auto">${startNum+i }</a>
						</c:if>
					</c:forEach>
					<c:if test="${startNum+5 < lastNum }">
						<a href="myPageUpload.do?myPageNum=${startNum+5 }" class="tm-paging-link col-md-auto">다음</a>
					</c:if>
					<c:if test="${startNum+5 >= lastNum }">
